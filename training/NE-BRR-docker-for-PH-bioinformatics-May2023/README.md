...
- Once you're finished working, make sure to shut down your workspace to save your allotted free hours in GitPod. There are a few ways to stop your workspace from running:
...
Now, run `docker ps --all` - do we see a container listed for the `ubuntu:focal` container we just ran? We should **not see it listed**, as long as the `docker run --rm` flag was included.
...
Files created inside of containers are not automatically exported from the container so we must use the `--volume <path-on-host>:<path-in-container>` or `-v <path-on-host>:<path-in-container>` (short hand). You must tell `docker` which path to mount from your host system, and where to mount the volume inside the container. It's OK if the path inside the container does not exist, it will be created.
